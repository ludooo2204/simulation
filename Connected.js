  ...
  return (
    <ScrollView horizontal={true} >
      {voieDeMesureFour.map((voie, i) => {
        ...
        <Text style={styles.voieName}>{voie.name+" - type "+voie.typeTc}</Text>
...
class ConnectionScreen extends React.Component {
    ...
    try {
      ...
      // this.test=new Date().toLocaleString('fr-FR', { month: 'long', day: 'numeric' })+"\n"
        ...
        setTimeout(()=>{ 
          ...
          // console.log(DataFourToTranfer.data);
          let stringToTransfert=DataFourToTranfer.nomFour+".."+DataFourToTranfer.date+".."+JSON.stringify(DataFourToTranfer.data);
          console.log(stringToTransfert);
          console.log("TTTTTTTTTTTTEEEEEEEEEEEEESSSSSSSSSSSSSSSSSSSSTTTTTTTTTTTT");
  ...
  handleInput(data){
   ...
   console.log(this.state.cleDataSave);
   let indexSimulation = (this.state.fourSelectionne.points.indexOf(parseInt(this.state.pointSimule)));
   ...
   console.log(indexSimulation);
    ...
    if (nextPoint){
      ...
      let stringVoievalidee=this.state.VoiesValidees+" "+this.state.voieSelectionne
...
//Selection de la voie
  selectVoie = (voie) => {
    ...
    let cleString=this.state.fourSelectionne.name+ "-"+this.state.voieSelectionne;
    ...
    setTimeout(()=>{this.SendDataAuto(consigne);console.log('et la consigne de depart apres 1 sec :'+consigne); console.log('this.myTextInput.current');this.myTextInput.current.focus();
    console.log('this.state.cleDataSave');
    console.log(this.state.cleDataSave);
  },100)},500)
    ...
    return (
      ...
      <View style={styles.container}>
        ...
        <View style={styles.listFourContainer}>
          <FourList  fourSelected={this.state.fourSelectionne} fours={this.state.fourList} onPress={this.selectFour} onLongPress={this.getAllData} />
          
        </View>
        <View style={{flex:10}}>
          {/* //TODO */}
          {/* <MyStack/> */}
          {this.state.fourSelectionne.name==undefined? (<Text>Choisissez un four</Text>) : 
          (
            
          <View style={{flex:1}}>

                  <View style = {{flex:1}} >
                         <VoieDeMesure  voieSelected= {this.state.voieSelectionne} voies={this.state.fourSelectionne} onPress={this.selectVoie}/>
                 </View>

                  <View style={{flex:3}}>
                    <Text style={{padding:5,textAlign:'center', fontSize:20}}>{"Points a controler  "+this.state.fourSelectionne.points }</Text> 
                    <View style={{alignItems:'center',justifyContent:'space-around',flexDirection:"row"}}>
					<Pressable onPress={this.decrementSensibilite}><Text style={{ paddingHorizontal:30, fontSize:70,fontWeight:"bold"}}>-</Text></Pressable>
                     
                        {this.state.pointSimule.includes("tcouple") ? (
             <ActivityIndicator size="small" color="black"/>) : (<Text style={{ padding:2,textAlign:'center', fontSize:40,borderWidth:3,borderRadius:10,width:"40%", fontWeight:'bold'}}>{this.state.pointSimule.includes("Func")?"-":this.state.pointSimule} °C</Text>)}
             
			 <Pressable onPress={this.incrementSensibilite}><Text style={{ paddingHorizontal:30,fontSize:70,fontWeight:"bold"}}>+</Text></Pressable>
                    </View>
                  </View>
                  ...
                  {/* </View> */}
                    <View style={{flex:3,justifyContent:"center",alignItems:"center"}}>
                      ...
                      <View style={{justifyContent:"center",alignItems:"center",margin:"5%",flexDirection:"row",borderWidth:3,borderRadius:10,width:"70%"}}>
                        <TextInput ref = {this.myTextInput}  autoFocus = {true} onBlur={()=>{console.log("blur")}} onLayout={()=>console.log('layout')}textAlign={'center'} blurOnSubmit={false} keyboardType='numeric'  onSubmitEditing={(e) => this.handleInput(e.nativeEvent.text)} style={styles.input} ></TextInput>
        ...
        <View style={[styles.horizontalContainer, {backgroundColor: '#ccc'}]}>
          ...
          returnKeyType="send"
          />
          <TouchableOpacity style={{justifyContent: 'center'}}    onPress={() => this.sendData()}>
            <Text>Send</Text>
          </TouchableOpacity>
        </View>
