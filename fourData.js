...
export let foursDataFromstorage = [];
let foursData = [
  {
    name: 'Minidiamant',
    id: 1,
...
const lireDATA = async (four) => {
  //   let keys = [];
  try {
    //  keys = await AsyncStorage.getAllKeys();
    const valueSaved = await AsyncStorage.getItem(four.name);

    if (valueSaved !== null) {
      let objectObtenu = JSON.parse(valueSaved);
    //   console.log(objectObtenu);
      foursDataFromstorage.push(objectObtenu);
    } else {
      // console.log(iterator.name);
      storeData(JSON.stringify(four)).then((e) => console.log('ca a amchéé'));
    }
  } catch (e) {
    // error reading value
    console.log('error');
...
const dataFromStorage = async () => {
  for (const four of foursData) {
    await lireDATA(four);
  }
};
const storeData = async (value) => {
  const nomDuFour = JSON.parse(value).name;
  try {
    // let Cle = this.state.fourSelectionne.name;

    await AsyncStorage.setItem(nomDuFour, value);
  } catch (e) {
    // saving error
    console.log(e);
  }
...
dataFromStorage().then(() => {
  console.log('foursDataFromstorage');
  console.log(foursDataFromstorage);
  return foursDataFromstorage
});
